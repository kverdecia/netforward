...
def coverage(c):
    ...
    """
    c.run("coverage run --source netforward runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/netforward.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ djformflows")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=netforward.__version__))
    c.run("git push --tags")
...
def publish(c):
    ...
    if os.path.isdir(dist_dir):
        c.run(f'rm -r {dist_dir}')
    if os.path.isdir(build_dir):
        c.run(f'rm -r {build_dir}')
    c.run('python setup.py sdist bdist_wheel')
    c.run('twine upload dist/*')
    import netforward
    version = netforward.__version__
